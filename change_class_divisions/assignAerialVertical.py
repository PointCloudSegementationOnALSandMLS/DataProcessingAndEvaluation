...
#points = np.load('Daten_Essen/Allee/grids/mobile/Lsplits/normalized/train/grid_normalized_5.npy')
    ...
    np.save("Daten_Essen/Allee/grids/aerial/vertical/grid_{}".format(i), complete)

points= np.load("./Daten_Essen/Allee/grids/aerial/normalized/test/test_grid.npy")
rawpoints = points[:,:3]
newClasses = np.copy(points[:,3])
print(newClasses)
mask = (newClasses == 7) 
...
print(newClasses)
complete = np.c_[ rawpoints, newClasses, points[:,4]]
np.save("Daten_Essen/Allee/grids/aerial/vertical/test_grid.npy", complete)